...
public interface OrdersRepository extends CrudRepository<OrdersEntity,Long> {
    ...
    @Query("SELECT COUNT(waiters.id) FROM OrdersEntity WHERE waiters.id=?1 and orderStatus=true and waiters.active=true")
    ...
    @Query("SELECT sum(orderPrice) FROM OrdersEntity WHERE waiters.id=?1 and orderStatus=true and waiters.active=true")
    Integer waiterRevenue(Long waitersId);

    @Query("select max(orderTime) from OrdersEntity where waiters.id=?1 and orderStatus=true and waiters.active=true")
    Date lastOrderWaiter(Long waitersId);

    @Query("select max(orderTime) from OrdersEntity where orderStatus=true and waiters.active=true")
    Date lastOrder();

    /*@Query("SELECT sum(orderPrice) FROM OrdersEntity WHERE orderTime>?1 and orderStatus=true")
    Integer sumRevenue(Date date);*/

    @Query("SELECT sum(orderPrice) FROM OrdersEntity WHERE orderStatus=true and waiters.active=true")
    Integer sumRevenue();

    /*@Query("SELECT COUNT(orderStatus) From OrdersEntity WHERE orderStatus=true and orderTime>?1")
    Integer sumOrdersClosed(Date date);*/

    @Query("SELECT COUNT(orderStatus) From OrdersEntity WHERE orderStatus=true and waiters.active=true")
    Integer sumOrdersClosed();

    @Query("SELECT AVG(orderPrice) from OrdersEntity WHERE waiters.id=?1 and waiters.active=true")
